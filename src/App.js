  ...
  const onAddToCart = async (obj) => {
     ...
     try {
      ...
      if (findItem) {
        ...
        await axios.delete(`https://624e91f177abd9e37c88446a.mockapi.io/cart/${findItem.id}`);
      } else {
        setCartItems((prev) => [...prev, obj]);
        const { data } = await axios.post('https://624e91f177abd9e37c88446a.mockapi.io/cart', obj);
        setCartItems((prev) =>
          prev.map((item) => {
  ...
  const onRemoveItem = (id) => {
    ...
    try {
      if (favorites.find((favObj) => Number(favObj.id) === Number(obj.id))) {
        ...
        const { data } = await axios.post('https://624e91f177abd9e37c88446a.mockapi.io/favorites',
          obj);
